/**
 * SyncClientV22 Add-on (MVP Push/Pull Closure) - act_acc_v199 v2
 * 修复点：鉴权方式兼容
 * - 同时发送 header: x-admin-token
 * - 同时在 URL 上追加 ?admin_token=xxx（兼容你早期服务端实现）
 */

const V22_CONFIG = {
  IDB_DB_NAME: "act_acc_v199",
  IDB_DB_VERSION: undefined,
  STORES: { accounts: "accounts", activities: "activities", entries: null },
  ID_FIELD: "id",
  LS_KEYS: { LOCAL_SNAPSHOT: "OPS_LOCAL_SNAPSHOT_V1", LAST_CLOUD_PULL: "OPS_LAST_CLOUD_PULL_V1" },
  API: { push: "/api/sync/push", pull: "/api/sync/pull", ping: "/api/sync/ping" }
};
...
function resolveAdminToken(){
  ...
  // 兼容：有些版本会把 token 放在 localStorage
  const ls = localStorage.getItem("ADMIN_SYNC_TOKEN") || localStorage.getItem("OPS_ADMIN_SYNC_TOKEN") || "";
  return ls || "";
}
...
async function postJSON(path, bodyObj){
  ...
  const url = withAdminToken(rawUrl, token);
  ...
  if (!res.ok || json?.ok === false){
    ...
    err.response = json;
    err._debug = { url, tokenPresent: !!token };
    throw err;
  }
  return json;
}
...
function computeDataHash(dataObj){ return hashString(stableStringify({data:dataObj})); }
...
function verifyClosure(localSnapshot, cloudWrapped){
  ...
  const countsOk = localCounts.accounts===cloudCounts.accounts && localCounts.activities===cloudCounts.activities && localCounts.entries===cloudCounts.entries;
...
function ensurePanel(){
  ...
  el.style.cssText = [
    ...
    "border-radius:12px","padding:12px","width:340px",
    "font:12px/1.4 system-ui, -apple-system, Segoe UI, Roboto, Arial"
  ].join(";");
  el.innerHTML = `
    <div style="font-weight:700;margin-bottom:6px;">V22 Push/Pull 闭环（v2 鉴权兼容）</div>
    <div style="color:#6b7280;margin-bottom:8px;">DB=act_acc_v199 stores=accounts,activities（entries为空）</div>
    ...
    <div id="v22-status" style="white-space:pre-wrap;color:#111827;background:#f9fafb;border:1px dashed #e5e7eb;border-radius:10px;padding:8px;min-height:92px;">就绪</div>
  `;
...
async function runClosure(){
  try{
    ...
    const dbg = e?._debug ? "\ndebug=" + JSON.stringify(e._debug) : "";
    setStatus(`闭环失败\n${e.message || e}${resp}${dbg}`);
    throw e;
  }
}
